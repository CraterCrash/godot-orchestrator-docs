...
|---------------------------|------------------------------------------------------------------------------------------------------------------------|
| AccessibilityServer       | Server interface for screen reader support.                                                                            |
...
| GDScriptLanguageProtocol  | GDScript language server - <EditorIcon name="NodeWarning"/> Experimental.                                              |
...
| JavaClassWrapper          | Provides access to the Java Native Interface (JNI).                                                                    |
...
| NavigationServer2DManager | A singleton for managing `NavigationServer2D` implementations.                                                         |
| NavigationServer3D        | A server interface for low-level 3D navigation access.                                                                 |
| NavigationServer3DManager | A singleton for managing `NavigationServer3D` implementations.                                                         |
...
There are two unique ways to place a **Singleton** node into the graph.

The first approach is to search for the **Singleton** by name in the **All Actions** dialog.
For example, if you want to place the `Time` singleton, simply search for `Time`:

<Figure image="/img/nodes/singletons/search-singletons.png" caption="Find singleton in the all actions dialog"></Figure>

The other approach is to place an **Engine Singleton** node into the graph.
The singleton node will automatically default to the `Engine` singleton.
